...
import com.hongguang.jaia.adapter.PingAnYiJianAdapter;
...
public class PingAnYiJianActivity extends Activity implements OnClickListener {
	...
	private PingAnYiJianAdapter adapter;
	...
	private String jihua_type[] = new String[]{"经济型", "全面性", "豪华型"};
	private String jihua_type_1[] = new String[]{"00820", "00821", "00822"};
	private TextView tv_baozhangjihua;
	private String jihua;

	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.ping_an_yi_jian);
		...
		// 保额和保费
		baoe = (TextView) findViewById(R.id.baoe_p);
		baoe.setText(baoe_1);

		// 计划类型
		tv_baozhangjihua = (TextView) findViewById(R.id.tv_baozhangjihua);
		tv_baozhangjihua.setOnClickListener(this);

		// 产品ID
		product_id = (TextView) findViewById(R.id.textView10);
		product_id.setText(product_id_1);
		// 保险的名字
		productname = (TextView) findViewById(R.id.productname_p);
		productname.setText(productname_1);
		// listView 以及adapter的显示
		productl = (LinearLayoutForListView) findViewById(R.id.prolist);
		adapter = new PingAnYiJianAdapter(this);
			...
			if (code == 200) {
				...
				try {
					...
					for (int i = 0; i < array.length(); i++) {
						...
						info = new ProductDetailinfo();
						info.setName(objtemp.optString("name"));
						info.setPlan1(objtemp.optString("plan1"));
						info.setPlan2(objtemp.optString("plan2"));
						info.setPlan3(objtemp.optString("plan3"));
						proinf.add(info);
					}
		...
		switch (v.getId()) {
			case R.id.btn_tj :
				if (cbxie.isChecked() == false) {
					...
					intent.putExtra("jihua", jihua == null ? "00820" : jihua);
			...
			case R.id.chakanxiangqing :
				...
				break;
			case R.id.tv_baozhangjihua :
				AlertDialog.Builder builder3 = new AlertDialog.Builder(this);
				builder3.setItems(jihua_type,
						new android.content.DialogInterface.OnClickListener() {

							@Override
							public void onClick(DialogInterface dialog,
									int which) {
								tv_baozhangjihua.setText(jihua_type[which]);
								jihua = jihua_type_1[which];
