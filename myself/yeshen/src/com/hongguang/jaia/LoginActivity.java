		...
		switch (v.getId()) {
			case R.id.login_btn :
				...
				if (TextUtils.isEmpty(sale.getPhone())) {
					Toast.makeText(LoginActivity.this, R.string.errorphone,
							Toast.LENGTH_LONG).show();
					return;
				}
				if (TextUtils.isEmpty(sale.getPassWord())) {
					Toast.makeText(LoginActivity.this, R.string.errorpass,
							Toast.LENGTH_LONG).show();
					return;
				}
				System.out.println("登录数据" + Constant.getGson().toJson(sale));
				params.add(new BasicNameValuePair("salesbean", Constant
						.getGson().toJson(sale)));
				new Thread(new HttpUtils(this, mHandler, WapConstant.URLSTRING
						+ WapConstant.LOGINString, params)).start();
		...
		public void handleMessage(android.os.Message msg) {
			...
			if (code == 200) {
				if (msg.obj.toString().trim().contains("sid")) {
					...
					dateSharedPreferences.saveLogin(LoginActivity.this,
							msg.obj.toString());
					Intent intent = new Intent();
					intent.setClass(LoginActivity.this, MainActivity.class);
					startActivity(intent);
				...
				Log.e("dong", code + "");
				Log.e("dong", msg.obj.toString());
				Toast.makeText(LoginActivity.this, R.string.longin_failed,
						Toast.LENGTH_LONG).show();
			}
		}
	};
